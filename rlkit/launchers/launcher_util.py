...
try:
    ...
    CODE_MOUNTS = [
        mount.MountLocal(local_dir=REPO_DIR, pythonpath=True, filter_dir=["data", "rlkit_venv", "s3_files"]),
    ]
    for code_dir in config.CODE_DIRS_TO_MOUNT:
        CODE_MOUNTS.append(mount.MountLocal(local_dir=code_dir, pythonpath=True, filter_dir=["data", "rlkit_venv", "s3_files"]))

    ...
    def default(self, o):
        if isinstance(o, type):
            ...
            if not o.__module__:
                o.__module__ = torch
            return {
                '$function': str(o.__module__) + "." + o.__name__
            }
...
class Logger(object):
    ...
    def update_header(self):
        """
        Copies old rows into current header scheme
        :return:
        """
        temp_file = NamedTemporaryFile(delete=False, mode="w")
        tabular_dict = dict(self._tabular)
        for csv_path in self._tabular_outputs:
            with open(csv_path, 'r') as old_csv:
                ...
                for row_dict in reader:
                    new_keys = (row_dict.keys() | set()).difference(tabular_dict.keys() | set())
                    for key in new_keys:
                        row_dict[key] = 0
                    writer.writerow(row_dict)

            # Close file in tabular_fds
            # Remove from tabular_outputs
            # Move temp file to csv_path
            # Readd to tabular_fds
            # Add to tabular_written
            self._tabular_fds[csv_path].close()
            self._tabular_fds.pop(csv_path)
            self._tabular_outputs.remove(csv_path)
            shutil.move(temp_file.name, csv_path)

            self.add_tabular_output(csv_path)
            self._tabular_header_written.add(self._tabular_fds[csv_path])
        del self._tabular[:]
